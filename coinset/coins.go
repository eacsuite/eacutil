...
func (cs *CoinSet) TotalValue() (value eacutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue eacutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue eacutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount eacutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue eacutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue eacutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue eacutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue eacutil.Amount, coins []Coin) (Coins, error) {
...
// eacutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() eacutil.Amount {
	return eacutil.Amount(c.txOut().Value)
}
