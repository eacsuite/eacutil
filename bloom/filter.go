...
// Filter defines a earthcoin bloom filter that provides easy manipulation of raw
// filter data.
type Filter struct {
	mtx           sync.Mutex
...
func (bf *Filter) hash(hashNum uint32, data []byte) uint32 {
	// earthcoind: 0xfba4c795 chosen as it guarantees a reasonable bit
...
//
// This function MUST be called with the filter lock held.
func (bf *Filter) matchTxAndUpdate(tx *eacutil.Tx) bool {
