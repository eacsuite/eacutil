...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := eacutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(eacutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := eacutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := eacutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
